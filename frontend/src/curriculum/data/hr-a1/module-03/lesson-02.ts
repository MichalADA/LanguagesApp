// AUTO-GENERATED by scripts/generate-a1-curriculum.mjs — nie edytuj ręcznie.
// a1-03-02 · Praca i nauka
// Źródło: curriculum/hr-a1/lexodromia_hr_A1_curriculum.csv (sha256 1173e95ad7caf5ae…) + didactics.json
import type { GeneratedLesson } from "../../../types";

export const LESSON: GeneratedLesson = {
  content: {
    "lessonId": "a1-03-02",
    "vocabulary": [
      {
        "target": "učiti",
        "source": "uczyć się",
        "lemma": "učiti",
        "partOfSpeech": "verb",
        "recordId": "A1-0200",
        "audioSrc": "/audio/hr/a1/module-03/uciti.mp3"
      },
      {
        "target": "studirati",
        "source": "studiować",
        "lemma": "studirati",
        "partOfSpeech": "verb",
        "recordId": "A1-0201",
        "audioSrc": "/audio/hr/a1/module-03/studirati.mp3"
      },
      {
        "target": "ured",
        "source": "biuro",
        "lemma": "ured",
        "partOfSpeech": "noun",
        "recordId": "A1-0202",
        "audioSrc": "/audio/hr/a1/module-03/ured.mp3"
      },
      {
        "target": "škola",
        "source": "szkoła",
        "lemma": "škola",
        "partOfSpeech": "noun",
        "recordId": "A1-0203",
        "audioSrc": "/audio/hr/a1/module-03/skola.mp3"
      },
      {
        "target": "fakultet",
        "source": "uczelnia / wydział",
        "lemma": "fakultet",
        "partOfSpeech": "noun",
        "recordId": "A1-0204",
        "audioSrc": "/audio/hr/a1/module-03/fakultet.mp3"
      },
      {
        "target": "kod kuće",
        "source": "w domu",
        "lemma": "kod kuće",
        "partOfSpeech": "phrase",
        "recordId": "A1-0205",
        "audioSrc": "/audio/hr/a1/module-03/kod-kuce.mp3"
      },
      {
        "target": "računalo",
        "source": "komputer",
        "lemma": "računalo",
        "partOfSpeech": "noun",
        "recordId": "A1-0206",
        "audioSrc": "/audio/hr/a1/module-03/racunalo.mp3"
      },
      {
        "target": "hrvatski",
        "source": "chorwacki",
        "lemma": "hrvatski",
        "partOfSpeech": "adjective",
        "recordId": "A1-0207",
        "audioSrc": "/audio/hr/a1/module-03/hrvatski.mp3"
      },
      {
        "target": "jezik",
        "source": "język",
        "lemma": "jezik",
        "partOfSpeech": "noun",
        "recordId": "A1-0802",
        "audioSrc": "/audio/hr/a1/module-03/jezik.mp3"
      },
      {
        "target": "engleski",
        "source": "angielski",
        "lemma": "engleski",
        "partOfSpeech": "adjective",
        "recordId": "A1-0803",
        "audioSrc": "/audio/hr/a1/module-03/engleski.mp3"
      },
      {
        "target": "poljski",
        "source": "polski",
        "lemma": "poljski",
        "partOfSpeech": "adjective",
        "recordId": "A1-0804",
        "audioSrc": "/audio/hr/a1/module-03/poljski.mp3"
      },
      {
        "target": "govoriti",
        "source": "mówić",
        "lemma": "govoriti",
        "partOfSpeech": "verb",
        "recordId": "A1-0805",
        "audioSrc": "/audio/hr/a1/module-03/govoriti.mp3"
      },
      {
        "target": "znati",
        "source": "wiedzieć / umieć",
        "lemma": "znati",
        "partOfSpeech": "verb",
        "recordId": "A1-0806",
        "audioSrc": "/audio/hr/a1/module-03/znati.mp3"
      },
      {
        "target": "tečaj",
        "source": "kurs",
        "lemma": "tečaj",
        "partOfSpeech": "noun",
        "recordId": "A1-0807",
        "audioSrc": "/audio/hr/a1/module-03/tecaj.mp3"
      },
      {
        "target": "lekcija",
        "source": "lekcja",
        "lemma": "lekcija",
        "partOfSpeech": "noun",
        "recordId": "A1-0808",
        "audioSrc": "/audio/hr/a1/module-03/lekcija.mp3"
      },
      {
        "target": "učenik",
        "source": "uczeń",
        "lemma": "učenik",
        "partOfSpeech": "noun",
        "recordId": "A1-0809",
        "audioSrc": "/audio/hr/a1/module-03/ucenik.mp3"
      },
      {
        "target": "malo",
        "source": "trochę",
        "lemma": "malo",
        "partOfSpeech": "adverb",
        "recordId": "A1-0811",
        "audioSrc": "/audio/hr/a1/module-03/malo.mp3"
      }
    ],
    "steps": [
      {
        "id": "intro",
        "stage": "intro",
        "type": "intro",
        "title": "Praca i nauka",
        "body": "Po tej lekcji będziesz umieć powiedzieć, gdzie pracujesz i czego się uczysz.",
        "goalsTitle": "Powiesz między innymi",
        "goals": [
          {
            "target": "Radim od kuće.",
            "source": "Pracuję z domu.",
            "audioSrc": "/audio/hr/a1/module-03/radim-od-kuce.mp3"
          },
          {
            "target": "Radim u uredu.",
            "source": "Pracuję w biurze.",
            "audioSrc": "/audio/hr/a1/module-03/radim-u-uredu.mp3"
          },
          {
            "target": "Učim hrvatski svaki dan.",
            "source": "Uczę się chorwackiego codziennie.",
            "audioSrc": "/audio/hr/a1/module-03/ucim-hrvatski-svaki-dan.mp3"
          }
        ]
      },
      {
        "id": "word-1",
        "stage": "words",
        "type": "word",
        "target": "učiti",
        "source": "uczyć się",
        "partOfSpeech": "czasownik",
        "example": {
          "target": "Učim hrvatski svaki dan.",
          "source": "Uczę się chorwackiego codziennie.",
          "audioSrc": "/audio/hr/a1/module-03/ucim-hrvatski-svaki-dan.mp3"
        },
        "audioSrc": "/audio/hr/a1/module-03/uciti.mp3"
      },
      {
        "id": "word-2",
        "stage": "words",
        "type": "word",
        "target": "studirati",
        "source": "studiować",
        "partOfSpeech": "czasownik",
        "example": {
          "target": "Studiram u Zagrebu.",
          "source": "Studiuję w Zagrzebiu.",
          "audioSrc": "/audio/hr/a1/module-03/studiram-u-zagrebu.mp3"
        },
        "audioSrc": "/audio/hr/a1/module-03/studirati.mp3"
      },
      {
        "id": "word-3",
        "stage": "words",
        "type": "word",
        "target": "ured",
        "source": "biuro",
        "partOfSpeech": "rzeczownik",
        "example": {
          "target": "Radim u uredu.",
          "source": "Pracuję w biurze.",
          "audioSrc": "/audio/hr/a1/module-03/radim-u-uredu.mp3"
        },
        "audioSrc": "/audio/hr/a1/module-03/ured.mp3"
      },
      {
        "id": "check-1",
        "stage": "words",
        "type": "choice",
        "instruction": "Co znaczy to słowo?",
        "prompt": "učiti",
        "options": [
          "uczyć się",
          "komputer",
          "szkoła"
        ],
        "correctIndex": 0,
        "targetText": "prompt",
        "promptAudioSrc": "/audio/hr/a1/module-03/uciti.mp3"
      },
      {
        "id": "word-4",
        "stage": "words",
        "type": "word",
        "target": "škola",
        "source": "szkoła",
        "partOfSpeech": "rzeczownik",
        "example": {
          "target": "Moj brat je učenik i ide u školu.",
          "source": "Mój brat jest uczniem i chodzi do szkoły.",
          "audioSrc": "/audio/hr/a1/module-03/moj-brat-je-ucenik-i-ide-u-skolu.mp3"
        },
        "audioSrc": "/audio/hr/a1/module-03/skola.mp3"
      },
      {
        "id": "word-5",
        "stage": "words",
        "type": "word",
        "target": "fakultet",
        "source": "uczelnia / wydział",
        "partOfSpeech": "rzeczownik",
        "audioSrc": "/audio/hr/a1/module-03/fakultet.mp3"
      },
      {
        "id": "word-6",
        "stage": "words",
        "type": "word",
        "target": "kod kuće",
        "source": "w domu",
        "partOfSpeech": "zwrot",
        "audioSrc": "/audio/hr/a1/module-03/kod-kuce.mp3"
      },
      {
        "id": "check-2",
        "stage": "words",
        "type": "choice",
        "instruction": "Jak powiesz to po chorwacku?",
        "prompt": "uczelnia / wydział",
        "options": [
          "škola",
          "fakultet",
          "učiti"
        ],
        "correctIndex": 1,
        "targetText": "options",
        "answerAudioSrc": "/audio/hr/a1/module-03/fakultet.mp3"
      },
      {
        "id": "word-7",
        "stage": "words",
        "type": "word",
        "target": "računalo",
        "source": "komputer",
        "partOfSpeech": "rzeczownik",
        "example": {
          "target": "Radim na računalu.",
          "source": "Pracuję na komputerze.",
          "audioSrc": "/audio/hr/a1/module-03/radim-na-racunalu.mp3"
        },
        "audioSrc": "/audio/hr/a1/module-03/racunalo.mp3"
      },
      {
        "id": "word-8",
        "stage": "words",
        "type": "word",
        "target": "hrvatski",
        "source": "chorwacki",
        "partOfSpeech": "przymiotnik",
        "example": {
          "target": "Učim hrvatski svaki dan.",
          "source": "Uczę się chorwackiego codziennie.",
          "audioSrc": "/audio/hr/a1/module-03/ucim-hrvatski-svaki-dan.mp3"
        },
        "audioSrc": "/audio/hr/a1/module-03/hrvatski.mp3"
      },
      {
        "id": "check-3",
        "stage": "words",
        "type": "choice",
        "instruction": "Co znaczy to słowo?",
        "prompt": "računalo",
        "options": [
          "chorwacki",
          "studiować",
          "komputer"
        ],
        "correctIndex": 2,
        "targetText": "prompt",
        "promptAudioSrc": "/audio/hr/a1/module-03/racunalo.mp3"
      },
      {
        "id": "more-words",
        "stage": "words",
        "type": "vocabList",
        "title": "Więcej przydatnych słów",
        "note": "Odsłuchaj i powtórz na głos. Te słowa trafią do fiszek razem z resztą lekcji.",
        "items": [
          {
            "target": "jezik",
            "source": "język",
            "partOfSpeech": "rzeczownik",
            "audioSrc": "/audio/hr/a1/module-03/jezik.mp3"
          },
          {
            "target": "engleski",
            "source": "angielski",
            "partOfSpeech": "przymiotnik",
            "audioSrc": "/audio/hr/a1/module-03/engleski.mp3"
          },
          {
            "target": "poljski",
            "source": "polski",
            "partOfSpeech": "przymiotnik",
            "audioSrc": "/audio/hr/a1/module-03/poljski.mp3"
          },
          {
            "target": "govoriti",
            "source": "mówić",
            "partOfSpeech": "czasownik",
            "audioSrc": "/audio/hr/a1/module-03/govoriti.mp3"
          },
          {
            "target": "znati",
            "source": "wiedzieć / umieć",
            "partOfSpeech": "czasownik",
            "audioSrc": "/audio/hr/a1/module-03/znati.mp3"
          },
          {
            "target": "tečaj",
            "source": "kurs",
            "partOfSpeech": "rzeczownik",
            "audioSrc": "/audio/hr/a1/module-03/tecaj.mp3"
          },
          {
            "target": "lekcija",
            "source": "lekcja",
            "partOfSpeech": "rzeczownik",
            "audioSrc": "/audio/hr/a1/module-03/lekcija.mp3"
          },
          {
            "target": "učenik",
            "source": "uczeń",
            "partOfSpeech": "rzeczownik",
            "audioSrc": "/audio/hr/a1/module-03/ucenik.mp3"
          },
          {
            "target": "malo",
            "source": "trochę",
            "partOfSpeech": "przysłówek",
            "audioSrc": "/audio/hr/a1/module-03/malo.mp3"
          }
        ]
      },
      {
        "id": "check-more",
        "stage": "words",
        "type": "choice",
        "instruction": "Co znaczy to słowo?",
        "prompt": "govoriti",
        "options": [
          "mówić",
          "trochę",
          "wiedzieć / umieć"
        ],
        "correctIndex": 0,
        "targetText": "prompt",
        "promptAudioSrc": "/audio/hr/a1/module-03/govoriti.mp3"
      },
      {
        "id": "examples",
        "stage": "words",
        "type": "structure",
        "title": "Nowe słowa w zdaniach",
        "explanation": "Posłuchaj, jak nowe słowa brzmią w krótkich, codziennych zdaniach.",
        "examples": [
          {
            "target": "Govorim poljski i engleski.",
            "source": "Mówię po polsku i po angielsku.",
            "audioSrc": "/audio/hr/a1/module-03/govorim-poljski-i-engleski.mp3"
          },
          {
            "target": "Govorim malo hrvatski.",
            "source": "Mówię trochę po chorwacku.",
            "audioSrc": "/audio/hr/a1/module-03/govorim-malo-hrvatski.mp3"
          },
          {
            "target": "Idem na tečaj hrvatskog jezika.",
            "source": "Chodzę na kurs języka chorwackiego.",
            "audioSrc": "/audio/hr/a1/module-03/idem-na-tecaj-hrvatskog-jezika.mp3"
          },
          {
            "target": "Moj brat je učenik i ide u školu.",
            "source": "Mój brat jest uczniem i chodzi do szkoły.",
            "audioSrc": "/audio/hr/a1/module-03/moj-brat-je-ucenik-i-ide-u-skolu.mp3"
          }
        ]
      },
      {
        "id": "structure",
        "stage": "structure",
        "type": "structure",
        "title": "Gdje radiš?",
        "explanation": "Na pytanie „gdzie?” odpowiadasz przez u i zmienioną końcówkę — tak jak po polsku: ured → u uredu (w biurze), Zagreb → u Zagrebu. Na razie ucz się tych zwrotów w całości. Dwa wyjątki do zapamiętania: kod kuće — w domu, od kuće — z domu.",
        "examples": [
          {
            "target": "Radim u uredu.",
            "source": "Pracuję w biurze.",
            "audioSrc": "/audio/hr/a1/module-03/radim-u-uredu.mp3"
          },
          {
            "target": "Studiram u Zagrebu.",
            "source": "Studiuję w Zagrzebiu.",
            "audioSrc": "/audio/hr/a1/module-03/studiram-u-zagrebu.mp3"
          },
          {
            "target": "Radim od kuće.",
            "source": "Pracuję z domu.",
            "audioSrc": "/audio/hr/a1/module-03/radim-od-kuce.mp3"
          }
        ]
      },
      {
        "id": "gap",
        "stage": "structure",
        "type": "gap",
        "instruction": "Uzupełnij lukę.",
        "before": "Radim u",
        "after": ".",
        "accepted": [
          "uredu"
        ],
        "translation": "Pracuję w biurze.",
        "answerAudioSrc": "/audio/hr/a1/module-03/radim-u-uredu.mp3"
      },
      {
        "id": "translate-1",
        "stage": "practice",
        "type": "translate",
        "instruction": "Przetłumacz na chorwacki.",
        "prompt": "Dziś nie pracuję.",
        "accepted": [
          "Danas ne radim.",
          "Ja danas ne radim.",
          "Ne radim danas."
        ],
        "answerAudioSrc": "/audio/hr/a1/module-03/danas-ne-radim.mp3"
      },
      {
        "id": "order",
        "stage": "practice",
        "type": "order",
        "instruction": "Ułóż zdanie.",
        "translation": "Uczę się chorwackiego codziennie.",
        "tokens": [
          "svaki",
          "dan",
          "hrvatski",
          "učim"
        ],
        "accepted": [
          "Učim hrvatski svaki dan.",
          "Ja učim hrvatski svaki dan."
        ],
        "answerAudioSrc": "/audio/hr/a1/module-03/ucim-hrvatski-svaki-dan.mp3"
      },
      {
        "id": "comprehend",
        "stage": "practice",
        "type": "choice",
        "instruction": "Co znaczy to zdanie?",
        "prompt": "Radim na računalu.",
        "options": [
          "Pracuję na komputerze.",
          "Pracuję z domu.",
          "Pracuję w biurze."
        ],
        "correctIndex": 0,
        "targetText": "prompt",
        "promptAudioSrc": "/audio/hr/a1/module-03/radim-na-racunalu.mp3"
      },
      {
        "id": "translate-more",
        "stage": "practice",
        "type": "translate",
        "instruction": "Przetłumacz na chorwacki.",
        "prompt": "Mówię trochę po chorwacku.",
        "accepted": [
          "Govorim malo hrvatski.",
          "Malo govorim hrvatski.",
          "Ja govorim malo hrvatski.",
          "Ja malo govorim hrvatski."
        ],
        "answerAudioSrc": "/audio/hr/a1/module-03/govorim-malo-hrvatski.mp3"
      },
      {
        "id": "model",
        "stage": "dialog",
        "type": "listen",
        "title": "Uczysz się czy pracujesz?",
        "lines": [
          {
            "speaker": "Marko",
            "text": "Govoriš li hrvatski?",
            "translation": "Mówisz po chorwacku?",
            "audioSrc": "/audio/hr/a1/module-03/govoris-li-hrvatski-m.mp3"
          },
          {
            "speaker": "Ana",
            "text": "Malo. Učim hrvatski na tečaju.",
            "translation": "Trochę. Uczę się chorwackiego na kursie.",
            "audioSrc": "/audio/hr/a1/module-03/malo-ucim-hrvatski-na-tecaju.mp3"
          },
          {
            "speaker": "Marko",
            "text": "A što radiš? Studiraš li?",
            "translation": "A co robisz? Studiujesz?",
            "audioSrc": "/audio/hr/a1/module-03/a-sto-radis-studiras-li-m.mp3"
          },
          {
            "speaker": "Ana",
            "text": "Ne, radim u uredu. A ti?",
            "translation": "Nie, pracuję w biurze. A ty?",
            "audioSrc": "/audio/hr/a1/module-03/ne-radim-u-uredu-a-ti.mp3"
          },
          {
            "speaker": "Marko",
            "text": "Ja studiram na fakultetu u Zagrebu.",
            "translation": "Ja studiuję na uczelni w Zagrzebiu.",
            "audioSrc": "/audio/hr/a1/module-03/ja-studiram-na-fakultetu-u-zagrebu-m.mp3"
          }
        ]
      },
      {
        "id": "dialog",
        "stage": "dialog",
        "type": "dialog",
        "title": "Praca i nauka",
        "turns": [
          {
            "kind": "line",
            "line": {
              "speaker": "Ana",
              "text": "Gdje radiš?",
              "translation": "Gdzie pracujesz?",
              "audioSrc": "/audio/hr/a1/module-03/gdje-radis.mp3"
            }
          },
          {
            "kind": "reply",
            "prompt": "Powiedz, gdzie pracujesz.",
            "accepted": [
              "Radim u uredu.",
              "Ja radim u uredu.",
              "Radim od kuće.",
              "Ja radim od kuće."
            ],
            "pattern": "^(?:(?:bok|zdravo|hej|dobar dan|dobro jutro|dobra večer|oprostite|hvala(?: lijepa| vam)?|ana) )*(?:((ja )?radim )?((u|na) (?:prijateljici|prijateljima|informaciji|ponavljanju|ponedjeljku|razglednici|rezervaciji|roditeljima|temperaturi|inozemstvu|konobarici|prijatelju|studentici|suncobranu|televiziji|tjestenini|učiteljici|aerodromu|apartmanu|fakultetu|gramatici|hrvatskoj|inženjeru|jelovniku|kašnjenju|kazalištu|kilogramu|kišobranu|knjižnici|kolodvoru|kupaonici|liječnici|liječniku|naočalama|napojnici|novčaniku|prodavaču|putovanju|putovnici|raskrižju|razgovoru|recepciji|restoranu|roditelju|sladoledu|autobusu|blagajni|četvrtku|čokoladi|domaćinu|gotovini|kolegici|koncertu|konobaru|krumpiru|ležaljci|limunadi|ljekarni|nedjelji|nogometu|obitelji|odgovoru|piletini|poljskoj|pomfritu|predjelu|prognozi|proljeću|prtljazi|računalu|rečenici|sastanku|semaforu|slušanju|studentu|suveniru|telefonu|teretani|trajektu|tramvaju|trgovini|učitelju|utakmici|zadataku|balkonu|biciklu|bolnici|čitanju|desertu|djetetu|dolasku|doručku|jogurtu|kartici|košarci|kuhinji|laptopu|lekciji|lozinci|maslacu|mjesecu|mlijeku|odbojci|pijesku|pisanju|pitanju|planini|polasku|prijavi|putniku|ručniku|ruksaku|snijegu|srijedi|stanici|stupnju|susjedi|susjedu|taksiju|tržnici|turistu|učeniku|ukrcaju|vikendu|vrećici|vremenu|adresi|avionu|bazenu|centru|cijeni|glazbi|godini|govoru|hobiju|hotelu|igrici|izlazu|izletu|jabuci|jeseni|jeziku|kafiću|ključu|knjizi|koferu|kolaču|kolezi|komadu|lijeku|minuti|mjestu|muzeju|odjavi|odmoru|pekari|peronu|pomoći|ponoći|poruci|povrću|računu|riječi|salati|šalici|šećeru|seriji|sestri|sportu|suboti|tečaju|tenisu|tjednu|tvrtci|utorku|večeri|večeru|vilici|vjetru|zabavi|zemlji|zgradi|banci|braći|bratu|brodu|broju|centu|crkvi|djeci|djedu|filmu|gradu|hrani|karti|kćeri|klimi|kruhu|liftu|ljetu|mačci|magli|majci|mostu|novcu|obali|oluji|osobi|otoku|parku|petku|planu|plaži|podnu|posau|pošti|ručku|školi|stanu|stolu|šunci|suncu|testu|torbi|ulazu|ulici|uredu|vlaku|autu|baci|boci|čaju|čaši|danu|euru|jaju|jelu|jozi|juhi|katu|kavi|kinu|kiši|kući|ledu|letu|mami|mesu|moru|mužu|noći|piću|pivu|poli|putu|ribi|riži|satu|sinu|siru|sobi|soku|tati|trgu|valu|vinu|voću|vodi|vrtu|ženi|zimi|imu|ocu|psu|wcu)|od kuće|kod kuće)|(ja )?(ne radim|studiram)( \\p{L}+){0,2})(?: (?:hvala(?: lijepa| vam)?|a ti|a vi|molim(?: vas)?|ana))*$",
            "suggestion": "Radim u uredu.",
            "suggestionAudioSrc": "/audio/hr/a1/module-03/radim-u-uredu.mp3"
          },
          {
            "kind": "line",
            "line": {
              "speaker": "Ana",
              "text": "Što učiš?",
              "translation": "Czego się uczysz?",
              "audioSrc": "/audio/hr/a1/module-03/sto-ucis.mp3"
            }
          },
          {
            "kind": "reply",
            "prompt": "Powiedz, że uczysz się chorwackiego.",
            "accepted": [
              "Učim hrvatski svaki dan.",
              "Ja učim hrvatski svaki dan.",
              "Učim hrvatski.",
              "Ja učim hrvatski."
            ],
            "pattern": "^(?:(?:bok|zdravo|hej|dobar dan|dobro jutro|dobra večer|oprostite|hvala(?: lijepa| vam)?|ana) )*(?:(ja )?učim hrvatski( jezik)?( svaki dan)?)(?: (?:hvala(?: lijepa| vam)?|a ti|a vi|molim(?: vas)?|ana))*$",
            "suggestion": "Učim hrvatski svaki dan.",
            "suggestionAudioSrc": "/audio/hr/a1/module-03/ucim-hrvatski-svaki-dan.mp3"
          }
        ]
      },
      {
        "id": "free",
        "stage": "dialog",
        "type": "free",
        "instruction": "Napisz, gdzie pracujesz lub się uczysz.",
        "points": [
          "praca albo nauka",
          "miejsce"
        ],
        "keywords": [
          {
            "any": [
              "radim",
              "studiram",
              "učim"
            ],
            "label": "praca lub nauka"
          },
          {
            "any": [
              "u",
              "od",
              "kod"
            ],
            "label": "miejsce (u … / od kuće)"
          }
        ],
        "minSentences": 2,
        "sample": "Radim u uredu. Učim hrvatski svaki dan.",
        "sampleAudioSrc": "/audio/hr/a1/module-03/radim-u-uredu-ucim-hrvatski-svaki-dan.mp3"
      },
      {
        "id": "summary",
        "stage": "summary",
        "type": "summary",
        "title": "Lekcja ukończona",
        "recap": [
          {
            "target": "Radim od kuće.",
            "source": "Pracuję z domu.",
            "audioSrc": "/audio/hr/a1/module-03/radim-od-kuce.mp3"
          },
          {
            "target": "Radim u uredu.",
            "source": "Pracuję w biurze.",
            "audioSrc": "/audio/hr/a1/module-03/radim-u-uredu.mp3"
          },
          {
            "target": "Učim hrvatski svaki dan.",
            "source": "Uczę się chorwackiego codziennie.",
            "audioSrc": "/audio/hr/a1/module-03/ucim-hrvatski-svaki-dan.mp3"
          },
          {
            "target": "Studiram u Zagrebu.",
            "source": "Studiuję w Zagrzebiu.",
            "audioSrc": "/audio/hr/a1/module-03/studiram-u-zagrebu.mp3"
          }
        ]
      }
    ]
  },
  material: {
    "sourceLessonId": "a1-12",
    "grammarFocus": "raditi/učiti/studirati; u + miejscownik w gotowych frazach",
    "communicativeGoal": "Powiedzieć, gdzie pracujesz i czego się uczysz.",
    "sources": [
      "https://www.coe.int/en/web/common-European-framework-reference-languages/table-1-cefr-3.3-common-reference-levels-global-scale",
      "https://www.coe.int/en/web/common-european-framework-reference-languages/cefr-descriptors-search",
      "https://croaticum.ffzg.unizg.hr/?page_id=1106",
      "https://a1.ffzg.unizg.hr/",
      "https://www.easy-croatian.com/2014/11/15.html",
      "https://www.easy-croatian.com/2014/11/6.html",
      "https://www.easy-croatian.com/2014/11/a4.html"
    ],
    "records": [
      {
        "recordId": "A1-0199",
        "type": "lesson",
        "sequence": 0,
        "hr": "",
        "pl": "",
        "lemma": "",
        "partOfSpeech": "",
        "exerciseType": "",
        "acceptedAnswers": [],
        "notes": "Projekt autorski oparty na deskryptorach CEFR A1 oraz zakresie początkujących kursów Croaticum. Treści nie są kopią podręcznika.",
        "tags": [
          "curriculum",
          "cefr-a1",
          "croatian"
        ]
      },
      {
        "recordId": "A1-0200",
        "type": "vocabulary",
        "sequence": 1,
        "hr": "učiti",
        "pl": "uczyć się",
        "lemma": "učiti",
        "partOfSpeech": "verb",
        "exerciseType": "flashcard|recognition|production",
        "acceptedAnswers": [
          "učiti"
        ],
        "notes": "Docelowy element aktywnego słownictwa; wrzuć do FSRS po ukończeniu lekcji.",
        "tags": [
          "vocab",
          "active"
        ]
      },
      {
        "recordId": "A1-0201",
        "type": "vocabulary",
        "sequence": 2,
        "hr": "studirati",
        "pl": "studiować",
        "lemma": "studirati",
        "partOfSpeech": "verb",
        "exerciseType": "flashcard|recognition|production",
        "acceptedAnswers": [
          "studirati"
        ],
        "notes": "Docelowy element aktywnego słownictwa; wrzuć do FSRS po ukończeniu lekcji.",
        "tags": [
          "vocab",
          "active"
        ]
      },
      {
        "recordId": "A1-0202",
        "type": "vocabulary",
        "sequence": 3,
        "hr": "ured",
        "pl": "biuro",
        "lemma": "ured",
        "partOfSpeech": "noun",
        "exerciseType": "flashcard|recognition|production",
        "acceptedAnswers": [
          "ured"
        ],
        "notes": "Docelowy element aktywnego słownictwa; wrzuć do FSRS po ukończeniu lekcji.",
        "tags": [
          "vocab",
          "active"
        ]
      },
      {
        "recordId": "A1-0203",
        "type": "vocabulary",
        "sequence": 4,
        "hr": "škola",
        "pl": "szkoła",
        "lemma": "škola",
        "partOfSpeech": "noun",
        "exerciseType": "flashcard|recognition|production",
        "acceptedAnswers": [
          "škola"
        ],
        "notes": "Docelowy element aktywnego słownictwa; wrzuć do FSRS po ukończeniu lekcji.",
        "tags": [
          "vocab",
          "active"
        ]
      },
      {
        "recordId": "A1-0204",
        "type": "vocabulary",
        "sequence": 5,
        "hr": "fakultet",
        "pl": "uczelnia / wydział",
        "lemma": "fakultet",
        "partOfSpeech": "noun",
        "exerciseType": "flashcard|recognition|production",
        "acceptedAnswers": [
          "fakultet"
        ],
        "notes": "Docelowy element aktywnego słownictwa; wrzuć do FSRS po ukończeniu lekcji.",
        "tags": [
          "vocab",
          "active"
        ]
      },
      {
        "recordId": "A1-0205",
        "type": "vocabulary",
        "sequence": 6,
        "hr": "kod kuće",
        "pl": "w domu",
        "lemma": "kod kuće",
        "partOfSpeech": "phrase",
        "exerciseType": "flashcard|recognition|production",
        "acceptedAnswers": [
          "kod kuće"
        ],
        "notes": "Docelowy element aktywnego słownictwa; wrzuć do FSRS po ukończeniu lekcji.",
        "tags": [
          "vocab",
          "active"
        ]
      },
      {
        "recordId": "A1-0206",
        "type": "vocabulary",
        "sequence": 7,
        "hr": "računalo",
        "pl": "komputer",
        "lemma": "računalo",
        "partOfSpeech": "noun",
        "exerciseType": "flashcard|recognition|production",
        "acceptedAnswers": [
          "računalo"
        ],
        "notes": "Docelowy element aktywnego słownictwa; wrzuć do FSRS po ukończeniu lekcji.",
        "tags": [
          "vocab",
          "active"
        ]
      },
      {
        "recordId": "A1-0207",
        "type": "vocabulary",
        "sequence": 8,
        "hr": "hrvatski",
        "pl": "chorwacki",
        "lemma": "hrvatski",
        "partOfSpeech": "adjective",
        "exerciseType": "flashcard|recognition|production",
        "acceptedAnswers": [
          "hrvatski"
        ],
        "notes": "Docelowy element aktywnego słownictwa; wrzuć do FSRS po ukończeniu lekcji.",
        "tags": [
          "vocab",
          "active"
        ]
      },
      {
        "recordId": "A1-0802",
        "type": "vocabulary",
        "sequence": 9,
        "hr": "jezik",
        "pl": "język",
        "lemma": "jezik",
        "partOfSpeech": "noun",
        "exerciseType": "flashcard|recognition",
        "acceptedAnswers": [
          "jezik"
        ],
        "notes": "Słownictwo uzupełniające: lista „Więcej przydatnych słów” w lekcji, nagranie i fiszki; bez osobnej karty słowa.",
        "tags": [
          "vocab",
          "supplement"
        ]
      },
      {
        "recordId": "A1-0803",
        "type": "vocabulary",
        "sequence": 10,
        "hr": "engleski",
        "pl": "angielski",
        "lemma": "engleski",
        "partOfSpeech": "adjective",
        "exerciseType": "flashcard|recognition",
        "acceptedAnswers": [
          "engleski"
        ],
        "notes": "Słownictwo uzupełniające: lista „Więcej przydatnych słów” w lekcji, nagranie i fiszki; bez osobnej karty słowa.",
        "tags": [
          "vocab",
          "supplement"
        ]
      },
      {
        "recordId": "A1-0804",
        "type": "vocabulary",
        "sequence": 11,
        "hr": "poljski",
        "pl": "polski",
        "lemma": "poljski",
        "partOfSpeech": "adjective",
        "exerciseType": "flashcard|recognition",
        "acceptedAnswers": [
          "poljski"
        ],
        "notes": "Słownictwo uzupełniające: lista „Więcej przydatnych słów” w lekcji, nagranie i fiszki; bez osobnej karty słowa.",
        "tags": [
          "vocab",
          "supplement"
        ]
      },
      {
        "recordId": "A1-0805",
        "type": "vocabulary",
        "sequence": 12,
        "hr": "govoriti",
        "pl": "mówić",
        "lemma": "govoriti",
        "partOfSpeech": "verb",
        "exerciseType": "flashcard|recognition",
        "acceptedAnswers": [
          "govoriti"
        ],
        "notes": "Słownictwo uzupełniające: lista „Więcej przydatnych słów” w lekcji, nagranie i fiszki; bez osobnej karty słowa.",
        "tags": [
          "vocab",
          "supplement"
        ]
      },
      {
        "recordId": "A1-0806",
        "type": "vocabulary",
        "sequence": 13,
        "hr": "znati",
        "pl": "wiedzieć / umieć",
        "lemma": "znati",
        "partOfSpeech": "verb",
        "exerciseType": "flashcard|recognition",
        "acceptedAnswers": [
          "znati"
        ],
        "notes": "Słownictwo uzupełniające: lista „Więcej przydatnych słów” w lekcji, nagranie i fiszki; bez osobnej karty słowa.",
        "tags": [
          "vocab",
          "supplement"
        ]
      },
      {
        "recordId": "A1-0807",
        "type": "vocabulary",
        "sequence": 14,
        "hr": "tečaj",
        "pl": "kurs",
        "lemma": "tečaj",
        "partOfSpeech": "noun",
        "exerciseType": "flashcard|recognition",
        "acceptedAnswers": [
          "tečaj"
        ],
        "notes": "Słownictwo uzupełniające: lista „Więcej przydatnych słów” w lekcji, nagranie i fiszki; bez osobnej karty słowa.",
        "tags": [
          "vocab",
          "supplement"
        ]
      },
      {
        "recordId": "A1-0808",
        "type": "vocabulary",
        "sequence": 15,
        "hr": "lekcija",
        "pl": "lekcja",
        "lemma": "lekcija",
        "partOfSpeech": "noun",
        "exerciseType": "flashcard|recognition",
        "acceptedAnswers": [
          "lekcija"
        ],
        "notes": "Słownictwo uzupełniające: lista „Więcej przydatnych słów” w lekcji, nagranie i fiszki; bez osobnej karty słowa.",
        "tags": [
          "vocab",
          "supplement"
        ]
      },
      {
        "recordId": "A1-0809",
        "type": "vocabulary",
        "sequence": 16,
        "hr": "učenik",
        "pl": "uczeń",
        "lemma": "učenik",
        "partOfSpeech": "noun",
        "exerciseType": "flashcard|recognition",
        "acceptedAnswers": [
          "učenik"
        ],
        "notes": "Słownictwo uzupełniające: lista „Więcej przydatnych słów” w lekcji, nagranie i fiszki; bez osobnej karty słowa.",
        "tags": [
          "vocab",
          "supplement"
        ]
      },
      {
        "recordId": "A1-0811",
        "type": "vocabulary",
        "sequence": 18,
        "hr": "malo",
        "pl": "trochę",
        "lemma": "malo",
        "partOfSpeech": "adverb",
        "exerciseType": "flashcard|recognition",
        "acceptedAnswers": [
          "malo"
        ],
        "notes": "Słownictwo uzupełniające: lista „Więcej przydatnych słów” w lekcji, nagranie i fiszki; bez osobnej karty słowa.",
        "tags": [
          "vocab",
          "supplement"
        ]
      },
      {
        "recordId": "A1-0208",
        "type": "sentence",
        "sequence": 1,
        "hr": "Radim od kuće.",
        "pl": "Pracuję z domu.",
        "lemma": "",
        "partOfSpeech": "",
        "exerciseType": "translation|word_order|listening|gap",
        "acceptedAnswers": [
          "Radim od kuće."
        ],
        "notes": "Zdanie autorskie do ćwiczeń; przy inputach tekstowych warto akceptować brak części znaków diakrytycznych jako soft error, ale pokazać poprawną pisownię.",
        "tags": [
          "sentence",
          "production",
          "comprehension"
        ]
      },
      {
        "recordId": "A1-0209",
        "type": "sentence",
        "sequence": 2,
        "hr": "Radim u uredu.",
        "pl": "Pracuję w biurze.",
        "lemma": "",
        "partOfSpeech": "",
        "exerciseType": "translation|word_order|listening|gap",
        "acceptedAnswers": [
          "Radim u uredu."
        ],
        "notes": "Zdanie autorskie do ćwiczeń; przy inputach tekstowych warto akceptować brak części znaków diakrytycznych jako soft error, ale pokazać poprawną pisownię.",
        "tags": [
          "sentence",
          "production",
          "comprehension"
        ]
      },
      {
        "recordId": "A1-0210",
        "type": "sentence",
        "sequence": 3,
        "hr": "Učim hrvatski svaki dan.",
        "pl": "Uczę się chorwackiego codziennie.",
        "lemma": "",
        "partOfSpeech": "",
        "exerciseType": "translation|word_order|listening|gap",
        "acceptedAnswers": [
          "Učim hrvatski svaki dan."
        ],
        "notes": "Zdanie autorskie do ćwiczeń; przy inputach tekstowych warto akceptować brak części znaków diakrytycznych jako soft error, ale pokazać poprawną pisownię.",
        "tags": [
          "sentence",
          "production",
          "comprehension"
        ]
      },
      {
        "recordId": "A1-0211",
        "type": "sentence",
        "sequence": 4,
        "hr": "Studiram u Zagrebu.",
        "pl": "Studiuję w Zagrzebiu.",
        "lemma": "",
        "partOfSpeech": "",
        "exerciseType": "translation|word_order|listening|gap",
        "acceptedAnswers": [
          "Studiram u Zagrebu."
        ],
        "notes": "Zdanie autorskie do ćwiczeń; przy inputach tekstowych warto akceptować brak części znaków diakrytycznych jako soft error, ale pokazać poprawną pisownię.",
        "tags": [
          "sentence",
          "production",
          "comprehension"
        ]
      },
      {
        "recordId": "A1-0212",
        "type": "sentence",
        "sequence": 5,
        "hr": "Radim na računalu.",
        "pl": "Pracuję na komputerze.",
        "lemma": "",
        "partOfSpeech": "",
        "exerciseType": "translation|word_order|listening|gap",
        "acceptedAnswers": [
          "Radim na računalu."
        ],
        "notes": "Zdanie autorskie do ćwiczeń; przy inputach tekstowych warto akceptować brak części znaków diakrytycznych jako soft error, ale pokazać poprawną pisownię.",
        "tags": [
          "sentence",
          "production",
          "comprehension"
        ]
      },
      {
        "recordId": "A1-0213",
        "type": "sentence",
        "sequence": 6,
        "hr": "Danas ne radim.",
        "pl": "Dziś nie pracuję.",
        "lemma": "",
        "partOfSpeech": "",
        "exerciseType": "translation|word_order|listening|gap",
        "acceptedAnswers": [
          "Danas ne radim."
        ],
        "notes": "Zdanie autorskie do ćwiczeń; przy inputach tekstowych warto akceptować brak części znaków diakrytycznych jako soft error, ale pokazać poprawną pisownię.",
        "tags": [
          "sentence",
          "production",
          "comprehension"
        ]
      },
      {
        "recordId": "A1-0812",
        "type": "sentence",
        "sequence": 7,
        "hr": "Govorim poljski i engleski.",
        "pl": "Mówię po polsku i po angielsku.",
        "lemma": "",
        "partOfSpeech": "",
        "exerciseType": "listening|comprehension",
        "acceptedAnswers": [
          "Govorim poljski i engleski."
        ],
        "notes": "Zdanie przykładowe z rozszerzenia modułów 02–08; pokazywane z nagraniem w kroku „Nowe słowa w zdaniach”.",
        "tags": [
          "sentence",
          "example",
          "supplement"
        ]
      },
      {
        "recordId": "A1-0813",
        "type": "sentence",
        "sequence": 8,
        "hr": "Govorim malo hrvatski.",
        "pl": "Mówię trochę po chorwacku.",
        "lemma": "",
        "partOfSpeech": "",
        "exerciseType": "listening|comprehension",
        "acceptedAnswers": [
          "Govorim malo hrvatski."
        ],
        "notes": "Zdanie przykładowe z rozszerzenia modułów 02–08; pokazywane z nagraniem w kroku „Nowe słowa w zdaniach”.",
        "tags": [
          "sentence",
          "example",
          "supplement"
        ]
      },
      {
        "recordId": "A1-0814",
        "type": "sentence",
        "sequence": 9,
        "hr": "Idem na tečaj hrvatskog jezika.",
        "pl": "Chodzę na kurs języka chorwackiego.",
        "lemma": "",
        "partOfSpeech": "",
        "exerciseType": "listening|comprehension",
        "acceptedAnswers": [
          "Idem na tečaj hrvatskog jezika."
        ],
        "notes": "Zdanie przykładowe z rozszerzenia modułów 02–08; pokazywane z nagraniem w kroku „Nowe słowa w zdaniach”.",
        "tags": [
          "sentence",
          "example",
          "supplement"
        ]
      },
      {
        "recordId": "A1-0815",
        "type": "sentence",
        "sequence": 10,
        "hr": "Moj brat je učenik i ide u školu.",
        "pl": "Mój brat jest uczniem i chodzi do szkoły.",
        "lemma": "",
        "partOfSpeech": "",
        "exerciseType": "listening|comprehension",
        "acceptedAnswers": [
          "Moj brat je učenik i ide u školu."
        ],
        "notes": "Zdanie przykładowe z rozszerzenia modułów 02–08; pokazywane z nagraniem w kroku „Nowe słowa w zdaniach”.",
        "tags": [
          "sentence",
          "example",
          "supplement"
        ]
      },
      {
        "recordId": "A1-0214",
        "type": "exercise_blueprint",
        "sequence": 1,
        "hr": "",
        "pl": "",
        "lemma": "",
        "partOfSpeech": "",
        "exerciseType": "multiple_choice",
        "acceptedAnswers": [],
        "notes": "Wybierz poprawne znaczenie lub odpowiedź do krótkiego promptu.",
        "tags": [
          "exercise",
          "lesson-player"
        ]
      },
      {
        "recordId": "A1-0215",
        "type": "exercise_blueprint",
        "sequence": 2,
        "hr": "",
        "pl": "",
        "lemma": "",
        "partOfSpeech": "",
        "exerciseType": "translation",
        "acceptedAnswers": [],
        "notes": "Przetłumacz 1 krótkie zdanie PL→HR, akceptując poprawne warianty.",
        "tags": [
          "exercise",
          "lesson-player"
        ]
      },
      {
        "recordId": "A1-0216",
        "type": "exercise_blueprint",
        "sequence": 3,
        "hr": "",
        "pl": "",
        "lemma": "",
        "partOfSpeech": "",
        "exerciseType": "free_response",
        "acceptedAnswers": [],
        "notes": "Napisz 1–2 własne zdania z materiałem lekcji; oceń sens i kluczową konstrukcję.",
        "tags": [
          "exercise",
          "lesson-player"
        ]
      }
    ]
  },
};
