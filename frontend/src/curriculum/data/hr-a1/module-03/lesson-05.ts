// AUTO-GENERATED by scripts/generate-a1-curriculum.mjs — nie edytuj ręcznie.
// a1-03-05 · Powtórka: mój tydzień
// Źródło: curriculum/hr-a1/lexodromia_hr_A1_curriculum.csv (sha256 1173e95ad7caf5ae…) + didactics.json
import type { GeneratedLesson } from "../../../types";

export const LESSON: GeneratedLesson = {
  content: {
    "lessonId": "a1-03-05",
    "vocabulary": [
      {
        "target": "tjedan",
        "source": "tydzień",
        "lemma": "tjedan",
        "partOfSpeech": "noun",
        "recordId": "A1-0254",
        "audioSrc": "/audio/hr/a1/module-03/tjedan.mp3"
      },
      {
        "target": "svaki dan",
        "source": "codziennie",
        "lemma": "svaki dan",
        "partOfSpeech": "phrase",
        "recordId": "A1-0255",
        "audioSrc": "/audio/hr/a1/module-03/svaki-dan.mp3"
      },
      {
        "target": "obično",
        "source": "zwykle",
        "lemma": "obično",
        "partOfSpeech": "adverb",
        "recordId": "A1-0256",
        "audioSrc": "/audio/hr/a1/module-03/obicno.mp3"
      },
      {
        "target": "ponekad",
        "source": "czasami",
        "lemma": "ponekad",
        "partOfSpeech": "adverb",
        "recordId": "A1-0257",
        "audioSrc": "/audio/hr/a1/module-03/ponekad.mp3"
      },
      {
        "target": "rano",
        "source": "wcześnie",
        "lemma": "rano",
        "partOfSpeech": "adverb",
        "recordId": "A1-0258",
        "audioSrc": "/audio/hr/a1/module-03/rano.mp3"
      },
      {
        "target": "kasno",
        "source": "późno",
        "lemma": "kasno",
        "partOfSpeech": "adverb",
        "recordId": "A1-0259",
        "audioSrc": "/audio/hr/a1/module-03/kasno.mp3"
      },
      {
        "target": "plan",
        "source": "plan",
        "lemma": "plan",
        "partOfSpeech": "noun",
        "recordId": "A1-0260",
        "audioSrc": "/audio/hr/a1/module-03/plan.mp3"
      },
      {
        "target": "slobodan",
        "source": "wolny",
        "lemma": "slobodan",
        "partOfSpeech": "adjective",
        "recordId": "A1-0261",
        "audioSrc": "/audio/hr/a1/module-03/slobodan.mp3"
      },
      {
        "target": "rijetko",
        "source": "rzadko",
        "lemma": "rijetko",
        "partOfSpeech": "adverb",
        "recordId": "A1-0845",
        "audioSrc": "/audio/hr/a1/module-03/rijetko.mp3"
      },
      {
        "target": "jednom tjedno",
        "source": "raz w tygodniu",
        "lemma": "tjedno",
        "partOfSpeech": "phrase",
        "recordId": "A1-0846",
        "audioSrc": "/audio/hr/a1/module-03/jednom-tjedno.mp3"
      },
      {
        "target": "svaki tjedan",
        "source": "co tydzień",
        "lemma": "tjedan",
        "partOfSpeech": "phrase",
        "recordId": "A1-0847",
        "audioSrc": "/audio/hr/a1/module-03/svaki-tjedan.mp3"
      },
      {
        "target": "mjesec",
        "source": "miesiąc",
        "lemma": "mjesec",
        "partOfSpeech": "noun",
        "recordId": "A1-0848",
        "audioSrc": "/audio/hr/a1/module-03/mjesec.mp3"
      },
      {
        "target": "radni dan",
        "source": "dzień roboczy",
        "lemma": "dan",
        "partOfSpeech": "phrase",
        "recordId": "A1-0849",
        "audioSrc": "/audio/hr/a1/module-03/radni-dan.mp3"
      }
    ],
    "steps": [
      {
        "id": "intro",
        "stage": "intro",
        "type": "intro",
        "title": "Powtórka: mój tydzień",
        "body": "Powtórka modułu „Codzienne życie”. Połączysz materiał z lekcji 11–14 i sprawdzisz go w rozmowie.",
        "goalsTitle": "Sprawdzisz, czy umiesz",
        "goals": [
          "opowiedzieć prostymi zdaniami o codziennych czynnościach",
          "powiedzieć, gdzie pracujesz i czego się uczysz",
          "zapytać o godzinę i podać godzinę",
          "powiedzieć, co robisz dziś, jutro i w wybrany dzień"
        ]
      },
      {
        "id": "recall-1",
        "stage": "words",
        "type": "translate",
        "instruction": "Jak powiesz to po chorwacku?",
        "prompt": "wstawać",
        "accepted": [
          "ustajati"
        ]
      },
      {
        "id": "recall-2",
        "stage": "words",
        "type": "translate",
        "instruction": "Jak powiesz to po chorwacku?",
        "prompt": "biuro",
        "accepted": [
          "ured"
        ],
        "answerAudioSrc": "/audio/hr/a1/module-03/ured.mp3"
      },
      {
        "id": "recall-3",
        "stage": "words",
        "type": "translate",
        "instruction": "Jak powiesz to po chorwacku?",
        "prompt": "jedenaście",
        "accepted": [
          "jedanaest"
        ],
        "answerAudioSrc": "/audio/hr/a1/module-03/jedanaest.mp3"
      },
      {
        "id": "recall-4",
        "stage": "words",
        "type": "choice",
        "instruction": "Co znaczy to słowo?",
        "prompt": "subota",
        "options": [
          "sobota",
          "uczelnia / wydział",
          "dokładnie"
        ],
        "correctIndex": 0,
        "targetText": "prompt",
        "promptAudioSrc": "/audio/hr/a1/module-03/subota.mp3"
      },
      {
        "id": "vocab",
        "stage": "words",
        "type": "vocabList",
        "title": "Przydatne słowa na koniec modułu",
        "note": "Nie musisz ich jeszcze znać na pamięć — pojawią się w rozmowie.",
        "items": [
          {
            "target": "tjedan",
            "source": "tydzień",
            "partOfSpeech": "rzeczownik",
            "audioSrc": "/audio/hr/a1/module-03/tjedan.mp3"
          },
          {
            "target": "svaki dan",
            "source": "codziennie",
            "partOfSpeech": "zwrot",
            "audioSrc": "/audio/hr/a1/module-03/svaki-dan.mp3"
          },
          {
            "target": "obično",
            "source": "zwykle",
            "partOfSpeech": "przysłówek",
            "audioSrc": "/audio/hr/a1/module-03/obicno.mp3"
          },
          {
            "target": "ponekad",
            "source": "czasami",
            "partOfSpeech": "przysłówek",
            "audioSrc": "/audio/hr/a1/module-03/ponekad.mp3"
          },
          {
            "target": "rano",
            "source": "wcześnie",
            "partOfSpeech": "przysłówek",
            "audioSrc": "/audio/hr/a1/module-03/rano.mp3"
          },
          {
            "target": "kasno",
            "source": "późno",
            "partOfSpeech": "przysłówek",
            "audioSrc": "/audio/hr/a1/module-03/kasno.mp3"
          },
          {
            "target": "plan",
            "source": "plan",
            "partOfSpeech": "rzeczownik",
            "audioSrc": "/audio/hr/a1/module-03/plan.mp3"
          },
          {
            "target": "slobodan",
            "source": "wolny",
            "partOfSpeech": "przymiotnik",
            "audioSrc": "/audio/hr/a1/module-03/slobodan.mp3"
          },
          {
            "target": "rijetko",
            "source": "rzadko",
            "partOfSpeech": "przysłówek",
            "audioSrc": "/audio/hr/a1/module-03/rijetko.mp3"
          },
          {
            "target": "jednom tjedno",
            "source": "raz w tygodniu",
            "partOfSpeech": "zwrot",
            "audioSrc": "/audio/hr/a1/module-03/jednom-tjedno.mp3"
          },
          {
            "target": "svaki tjedan",
            "source": "co tydzień",
            "partOfSpeech": "zwrot",
            "audioSrc": "/audio/hr/a1/module-03/svaki-tjedan.mp3"
          },
          {
            "target": "mjesec",
            "source": "miesiąc",
            "partOfSpeech": "rzeczownik",
            "audioSrc": "/audio/hr/a1/module-03/mjesec.mp3"
          },
          {
            "target": "radni dan",
            "source": "dzień roboczy",
            "partOfSpeech": "zwrot",
            "audioSrc": "/audio/hr/a1/module-03/radni-dan.mp3"
          }
        ]
      },
      {
        "id": "examples",
        "stage": "words",
        "type": "structure",
        "title": "Nowe słowa w zdaniach",
        "explanation": "Posłuchaj, jak nowe słowa brzmią w krótkich, codziennych zdaniach.",
        "examples": [
          {
            "target": "Rijetko radim vikendom.",
            "source": "Rzadko pracuję w weekendy.",
            "audioSrc": "/audio/hr/a1/module-03/rijetko-radim-vikendom.mp3"
          },
          {
            "target": "Jednom tjedno idem na bazen.",
            "source": "Raz w tygodniu chodzę na basen.",
            "audioSrc": "/audio/hr/a1/module-03/jednom-tjedno-idem-na-bazen.mp3"
          }
        ]
      },
      {
        "id": "gap",
        "stage": "structure",
        "type": "gap",
        "instruction": "Uzupełnij lukę.",
        "before": "U",
        "after": "se odmaram.",
        "accepted": [
          "nedjelju"
        ],
        "translation": "W niedzielę odpoczywam.",
        "answerAudioSrc": "/audio/hr/a1/module-03/u-nedjelju-se-odmaram.mp3"
      },
      {
        "id": "order",
        "stage": "structure",
        "type": "order",
        "instruction": "Ułóż zdanie.",
        "translation": "Najpierw biorę prysznic, potem jem śniadanie.",
        "tokens": [
          "se",
          "tuširam",
          "doručkujem",
          "prvo",
          "zatim"
        ],
        "accepted": [
          "Prvo se tuširam, zatim doručkujem."
        ],
        "answerAudioSrc": "/audio/hr/a1/module-03/prvo-se-tusiram-zatim-doruckujem.mp3"
      },
      {
        "id": "translate-1",
        "stage": "practice",
        "type": "translate",
        "instruction": "Przetłumacz na chorwacki.",
        "prompt": "Wstaję o siódmej.",
        "accepted": [
          "Ustajem u sedam.",
          "Ja ustajem u sedam."
        ],
        "answerAudioSrc": "/audio/hr/a1/module-03/ustajem-u-sedam.mp3"
      },
      {
        "id": "translate-2",
        "stage": "practice",
        "type": "translate",
        "instruction": "Przetłumacz na chorwacki.",
        "prompt": "Pracuję z domu.",
        "accepted": [
          "Radim od kuće.",
          "Ja radim od kuće."
        ],
        "answerAudioSrc": "/audio/hr/a1/module-03/radim-od-kuce.mp3"
      },
      {
        "id": "translate-3",
        "stage": "practice",
        "type": "translate",
        "instruction": "Przetłumacz na chorwacki.",
        "prompt": "Dziś jest czwartek.",
        "accepted": [
          "Danas je četvrtak."
        ],
        "answerAudioSrc": "/audio/hr/a1/module-03/danas-je-cetvrtak.mp3"
      },
      {
        "id": "dialog",
        "stage": "dialog",
        "type": "dialog",
        "title": "Mój tydzień",
        "turns": [
          {
            "kind": "line",
            "line": {
              "speaker": "Marko",
              "text": "Što radiš svaki dan?",
              "translation": "Co robisz codziennie?",
              "audioSrc": "/audio/hr/a1/module-03/sto-radis-svaki-dan-m.mp3"
            }
          },
          {
            "kind": "reply",
            "prompt": "Powiedz, że codziennie uczysz się chorwackiego.",
            "accepted": [
              "Svaki dan učim hrvatski.",
              "Ja svaki dan učim hrvatski.",
              "Učim hrvatski svaki dan.",
              "Ja učim hrvatski svaki dan."
            ],
            "pattern": "^(?:(?:bok|zdravo|hej|dobar dan|dobro jutro|dobra večer|oprostite|hvala(?: lijepa| vam)?|marko) )*(?:(svaki dan (ja )?učim hrvatski( jezik)?|(ja )?učim hrvatski( jezik)? svaki dan))(?: (?:hvala(?: lijepa| vam)?|a ti|a vi|molim(?: vas)?|marko))*$",
            "suggestion": "Svaki dan učim hrvatski.",
            "suggestionAudioSrc": "/audio/hr/a1/module-03/svaki-dan-ucim-hrvatski.mp3"
          },
          {
            "kind": "line",
            "line": {
              "speaker": "Marko",
              "text": "Kada radiš?",
              "translation": "Kiedy pracujesz?",
              "audioSrc": "/audio/hr/a1/module-03/kada-radis-m.mp3"
            }
          },
          {
            "kind": "reply",
            "prompt": "Powiedz, od której zwykle pracujesz.",
            "accepted": [
              "Obično radim od deset.",
              "Ja obično radim od deset.",
              "Radim od deset.",
              "Ja radim od deset."
            ],
            "pattern": "^(?:(?:bok|zdravo|hej|dobar dan|dobro jutro|dobra večer|oprostite|hvala(?: lijepa| vam)?|marko) )*(?:((obično )?(ja )?(obično )?radim )?(od \\p{L}+( do \\p{L}+)?|u \\p{L}+)( sati)?)(?: (?:hvala(?: lijepa| vam)?|a ti|a vi|molim(?: vas)?|marko))*$",
            "suggestion": "Obično radim od deset.",
            "suggestionAudioSrc": "/audio/hr/a1/module-03/obicno-radim-od-deset.mp3"
          },
          {
            "kind": "line",
            "line": {
              "speaker": "Marko",
              "text": "Jesi li slobodan u subotu?",
              "translation": "Jesteś wolny w sobotę?",
              "audioSrc": "/audio/hr/a1/module-03/jesi-li-slobodan-u-subotu-m.mp3"
            }
          },
          {
            "kind": "reply",
            "prompt": "Powiedz, że w sobotę jesteś wolny.",
            "accepted": [
              "U subotu sam slobodan.",
              "U subotu sam slobodna.",
              "Da, u subotu sam slobodan.",
              "Da, u subotu sam slobodna.",
              "Da, slobodan sam.",
              "Da, slobodna sam."
            ],
            "pattern": "^(?:(?:bok|zdravo|hej|dobar dan|dobro jutro|dobra večer|oprostite|hvala(?: lijepa| vam)?|marko) )*(?:(da )?(u subotu sam (?:slobodan|slobodna)|(?:slobodan|slobodna) sam( u subotu)?|ja sam (?:slobodan|slobodna)( u subotu)?))(?: (?:hvala(?: lijepa| vam)?|a ti|a vi|molim(?: vas)?|marko))*$",
            "suggestion": "U subotu sam slobodan.",
            "suggestionAudioSrc": "/audio/hr/a1/module-03/u-subotu-sam-slobodan.mp3"
          },
          {
            "kind": "line",
            "line": {
              "speaker": "Marko",
              "text": "Što radiš navečer?",
              "translation": "Co robisz wieczorem?",
              "audioSrc": "/audio/hr/a1/module-03/sto-radis-navecer-m.mp3"
            }
          },
          {
            "kind": "reply",
            "prompt": "Powiedz, co zwykle robisz wieczorem.",
            "accepted": [
              "Navečer se obično odmaram.",
              "Navečer učim hrvatski.",
              "Ja navečer učim hrvatski.",
              "Navečer obično spavam.",
              "Ja navečer obično spavam."
            ],
<<<<<<< HEAD
            "pattern": "^(?:(?:bok|zdravo|hej|dobar dan|dobro jutro|dobra večer|oprostite|hvala(?: lijepa| vam)?|marko) )*(?:((navečer|obično|često|ponekad|uvijek|rijetko) )*(ja )?((navečer|obično|često|ponekad|uvijek|rijetko) )*((?!volim )(?:idem u kupovinu|platim karticom|imam vremena|preporučujem|vozim bicikl|sastanem se|se sastanem|doručkujem|izgubim se|odmaram se|odmorim se|planinarim|preporučim|rezerviram|se izgubim|se odmaram|se odmorim|se tuširam|tuširam se|odgovorim|presjedam|razumijem|se sunčam|sunčam se|budim se|kupam se|nadam se|nađem se|odaberem|planiram|posjetim|poslušam|poznajem|prijeđem|pronađem|se budim|se kupam|se nadam|se nađem|se zovem|studiram|zovem se|govorim|izlazim|napišem|naručim|pokažem|ponovim|putujem|skrenem|stignem|ustajem|večeram|vježbam|završim|gledam|kasnim|koštam|krenem|opišem|pjevam|platim|plešem|plivam|pričam|skijam|slušam|spavam|stižem|tražim|trebam|čitam|crtam|igram|jedem|kuham|kupim|nemam|pijem|pitam|radim|ronim|ručam|šetam|trčim|vidim|volim|vozim|želim|živim|hoću|idem|imam|učim|znam)|se (?:idem u kupovinu|platim karticom|imam vremena|preporučujem|vozim bicikl|sastanem se|se sastanem|doručkujem|izgubim se|odmaram se|odmorim se|planinarim|preporučim|rezerviram|se izgubim|se odmaram|se odmorim|se tuširam|tuširam se|odgovorim|presjedam|razumijem|se sunčam|sunčam se|budim se|kupam se|nadam se|nađem se|odaberem|planiram|posjetim|poslušam|poznajem|prijeđem|pronađem|se budim|se kupam|se nadam|se nađem|se zovem|studiram|zovem se|govorim|izlazim|napišem|naručim|pokažem|ponovim|putujem|skrenem|stignem|ustajem|večeram|vježbam|završim|gledam|kasnim|koštam|krenem|opišem|pjevam|platim|plešem|plivam|pričam|skijam|slušam|spavam|stižem|tražim|trebam|čitam|crtam|igram|jedem|kuham|kupim|nemam|pijem|pitam|radim|ronim|ručam|šetam|trčim|vidim|volim|vozim|želim|živim|hoću|idem|imam|učim|znam)|volim ((?:platiti karticom|sljedeći tjedan|ići u kupovinu|imati vremena|kupaći kostim|voziti bicikl|doručkovati|izgubiti se|odmarati se|odmoriti se|planinariti|preporučiti|rezervirati|se izgubiti|se odmarati|se odmoriti|se tuširati|tuširati se|odgovoriti|presjedati|sastati se|se sastati|se sunčati|sunčati se|buditi se|kupati se|nadati se|planirati|posjetiti|poslušati|poznavati|razumjeti|se buditi|se kupati|se nadati|studirati|govoriti|izlaziti|napisati|naručiti|odabrati|pokazati|ponoviti|putovati|se zvati|skrenuti|ustajati|večerati|vježbati|završiti|zvati se|gledati|kasniti|koštati|krenuti|naći se|opisati|oprosti|pjevati|platiti|plesati|plivati|pričati|prijeći|pronaći|se naći|skijati|slušati|spavati|tražiti|trebati|vidjeti|voljeti|željeti|živjeti|čitati|crtati|htjeti|igrati|kuhati|kupiti|nemati|pitati|raditi|roniti|ručati|šetati|trčati|voziti|imati|izaći|jesti|stići|učiti|znati|piti|ići)|(?:zajedno ili odvojeno|autobusni kolodvor|jednokrevetnu sobu|kremu za sunčanje|do centra, molim|dvokrevetnu sobu|osobnu iskaznicu|slobodno vrijeme|hvalu na pomoći|turistički ured|u jednom smjeru|godišnji odmor|mogu li dobiti|nu kraju ulice|povratnu kartu|zu tjedan danu|boli me glavu|jednom tjedno|nemam vremenu|nemu problemu|prošli tjedan|stol za dvoje|u koliko sati|hvalu lijepu|kako se kaže|ne razumijem|nemu na čemu|prijateljice|prijateljicu|prijateljiji|svaki tjedan|bijelu kavu|dobro jutro|dobru večer|drago mi je|glavno jelo|informacije|informaciju|ponavljanja|ponavljanje|ponedjeljak|ponedjeljke|putu tjedno|razglednice|razglednicu|rezervacije|rezervaciju|roditeljiji|temperature|temperaturu|zvuči super|cijeli dan|inozemstva|inozemstvo|još jednom|konobarice|konobaricu|plan gradu|prijatelja|prijatelje|sretan put|studentice|studenticu|suncobrane|televizije|televiziju|tjestenine|tjesteninu|učiteljice|učiteljicu|aerodrome|apartmane|dobar dan|dobar tek|fakultete|gramatike|gramatiku|hoćemo li|inženjera|inženjere|jelovnike|kašnjenja|kašnjenje|kazališta|kazalište|kilograme|kišobrane|knjižnice|knjižnicu|kolodvore|kupaonice|kupaonicu|liječnice|liječnicu|liječnika|liječnike|molim vas|napojnice|napojnicu|novčanike|prodavača|prodavače|putovanja|putovanje|putovnice|putovnicu|radni dan|raskrižja|raskrižje|razgovore|recepcije|recepciju|restorane|roditelje|sladolede|što radiš|što znači|suncobran|svaki dan|to je sve|vidimo se|vozni red|aerodrom|apartman|autobuse|blagajne|blagajnu|četvrtak|četvrtke|čokolade|čokoladu|domaćine|fakultet|gotovine|gotovinu|hoćeš li|hrvatske|hrvatsku|imate li|jelovnik|kako ste|kilogram|kišobran|ključeve|kod kuće|kolegice|kolegicu|kolodvor|koncerte|konobara|konobare|krumpire|laku noć|ležaljke|ležaljku|limunade|limunadu|ljekarne|ljekarnu|nedjelje|nedjelju|nogomete|novčanik|obitelji|odgovore|piletine|piletinu|pomfrite|predjela|predjelo|prognoze|prognozu|proljeća|proljeće|prtljage|prtljagu|računala|računalo|razgovor|rečenice|rečenicu|restoran|roditelj|sastanak|sastanke|semafore|sladoled|slušanja|slušanje|sportove|studenta|studente|suvenire|telefone|teretane|teretanu|trajekte|tramvaje|trgovine|trgovinu|učitelja|učitelje|utakmice|utakmicu|zadatake|autobus|balkone|bicikle|bolnice|bolnicu|bratove|brodove|brojeve|centove|čitanja|čitanje|deserte|djedove|dolaske|dolazak|domaćin|doručak|doručke|filmove|gradove|javi se|jogurte|kartice|karticu|koncert|košarke|košarku|kruhove|krumpir|kuhinje|kuhinju|laptope|lekcije|lekciju|liftove|lozinke|lozinku|maslace|mjesece|mlijeka|mlijeko|mostove|naočala|naočale|ne mogu|ne znam|nogomet|nu piće|nu uglu|obitelj|odbojke|odbojku|odgovor|parkove|pijesak|pijeske|pisanja|pisanje|pitanja|pitanje|planine|planinu|planove|polaske|polazak|poljske|poljsku|pomfrit|prijave|prijavu|putnika|putnike|ručnike|ruksake|semafor|snijege|srijede|srijedu|stanice|stanicu|stanove|stolove|stupanj|stupnje|susjeda|susjede|susjedu|suvenir|taksiji|telefon|testove|trajekt|tramvaj|tržnice|tržnicu|turista|turiste|učenika|učenike|ukrcaje|vikende|vlakove|vrećice|vrećicu|vrijema|vrijeme|zadatak|adrese|adresu|avione|balkon|bazene|bicikl|čajeve|centar|centre|cijene|cijenu|danove|desert|dijete|glazbe|glazbu|godine|godinu|govore|hobiji|hotele|igrice|igricu|izlaze|izlete|jabuke|jabuku|jeseni|jezike|jogurt|kafiće|katove|knjige|knjigu|kofere|kolače|kolege|kolegu|komade|laptop|ledove|letove|lijeke|maslac|minute|minutu|mjesec|mjesta|mjesto|muzeje|muževe|odjave|odjavu|odmore|pekare|pekaru|perone|pomoći|ponoći|poruke|poruku|povrća|povrće|putove|račune|riječi|ručnik|ruksak|salate|salatu|šalice|šalicu|satove|šećere|serije|seriju|sestre|sestru|sinove|sirove|snijeg|sokove|subote|subotu|tečaje|tenise|tjedan|tjedne|tvrtke|tvrtku|u šest|ukrcaj|utorak|utorke|valove|večere|večeru|vikend|vilice|vilicu|vjetar|vjetre|zabave|zabavu|zemlje|zemlju|zgrade|zgradu|avion|banke|banku|bazen|braće|braću|brata|crkve|crkvu|djece|djecu|djeda|govor|hotel|hrane|hranu|izlaz|izlet|jesen|jezik|kafić|karte|kartu|klime|klimu|ključ|kofer|kolač|komad|lijek|ljeta|ljeto|mačke|mačku|magle|maglu|majke|majku|muzej|novac|novce|obale|obalu|odmor|oluje|oluju|osobe|osobu|otoke|peron|petak|petke|plaže|plažu|podna|podne|pomoć|ponoć|posaa|posao|pošte|poštu|račun|riječ|ručak|ručke|šećer|škole|školu|sport|sunca|sunce|šunke|šunku|taksi|tečaj|tenis|torbe|torbu|ulaze|ulice|ulicu|urede|večer|auta|auto|bake|baku|boce|bocu|brod|broj|čaše|čašu|cent|eura|euro|film|grad|hobi|i ju|jaja|jaje|jela|jelo|joge|jogu|juhe|juhu|kave|kavu|kćer|kćii|kina|kino|kiše|kišu|kruh|kuće|kuću|lift|mame|mamu|mesa|meso|mora|more|most|muža|noći|otok|park|pića|piće|piva|pivo|plan|pole|polu|ribe|ribu|riže|rižu|sina|sobe|sobu|stan|stol|tate|tatu|test|trge|u ti|ulaz|ured|vina|vino|vlak|voća|voće|vode|vodu|vrte|žene|ženu|zime|zimu|čaj|dan|ima|ime|kat|led|let|noć|oca|oce|psa|pse|put|sat|sir|sok|trg|val|vrt|wce|wc)))( \\p{L}+){0,3}( (navečer|obično|često|ponekad|uvijek|rijetko))*|((navečer|obično|često|ponekad|uvijek|rijetko) )*(ja )?se ((navečer|obično|često|ponekad|uvijek|rijetko) )+(?:idem u kupovinu|platim karticom|imam vremena|preporučujem|vozim bicikl|doručkujem|planinarim|preporučim|rezerviram|odgovorim|presjedam|razumijem|odaberem|planiram|posjetim|poslušam|poznajem|prijeđem|pronađem|sastanem|studiram|govorim|izgubim|izlazim|napišem|naručim|odmaram|odmorim|pokažem|ponovim|putujem|skrenem|stignem|tuširam|ustajem|večeram|vježbam|završim|gledam|kasnim|koštam|krenem|opišem|pjevam|platim|plešem|plivam|pričam|skijam|slušam|spavam|stižem|sunčam|tražim|trebam|budim|čitam|crtam|igram|jedem|kuham|kupam|kupim|nadam|nađem|nemam|pijem|pitam|radim|ronim|ručam|šetam|trčim|vidim|volim|vozim|želim|živim|zovem|hoću|idem|imam|učim|znam)( \\p{L}+){0,3})(?: (?:hvala(?: lijepa| vam)?|a ti|a vi|molim(?: vas)?|marko))*$",
            "suggestion": "Navečer se obično odmaram."
=======
            "pattern": "^(?:(?:bok|zdravo|hej|dobar dan|dobro jutro|dobra večer|oprostite|hvala(?: lijepa| vam)?|marko) )*(?:((navečer|obično|često|ponekad|uvijek|rijetko) )*(ja )?((navečer|obično|često|ponekad|uvijek|rijetko) )*((?!volim )(?:idem u kupovinu|platim karticom|fotografiram|imam vremena|preporučujem|vozim bicikl|sastanem se|se sastanem|doručkujem|izgubim se|odmaram se|odmorim se|planinarim|preporučim|rezerviram|se izgubim|se odmaram|se odmorim|se tuširam|tuširam se|odgovorim|presjedam|razumijem|se sunčam|sunčam se|budim se|kupam se|nadam se|nađem se|odaberem|planiram|posjetim|poslušam|poznajem|prijeđem|pronađem|se budim|se kupam|se nadam|se nađem|se zovem|studiram|zovem se|govorim|izlazim|napišem|naručim|pokažem|ponovim|putujem|skrenem|stignem|ustajem|večeram|vježbam|završim|gledam|kasnim|koštam|krenem|opišem|pjevam|platim|plešem|plivam|poznam|pričam|skijam|slušam|spavam|stižem|tražim|trebam|čitam|crtam|igram|jedem|kuham|kupim|nemam|pijem|pitam|radim|ronim|ručam|šetam|trčim|vidim|volim|vozim|želim|živim|hoću|idem|imam|učim|znam)|se (?:idem u kupovinu|platim karticom|fotografiram|imam vremena|preporučujem|vozim bicikl|sastanem se|se sastanem|doručkujem|izgubim se|odmaram se|odmorim se|planinarim|preporučim|rezerviram|se izgubim|se odmaram|se odmorim|se tuširam|tuširam se|odgovorim|presjedam|razumijem|se sunčam|sunčam se|budim se|kupam se|nadam se|nađem se|odaberem|planiram|posjetim|poslušam|poznajem|prijeđem|pronađem|se budim|se kupam|se nadam|se nađem|se zovem|studiram|zovem se|govorim|izlazim|napišem|naručim|pokažem|ponovim|putujem|skrenem|stignem|ustajem|večeram|vježbam|završim|gledam|kasnim|koštam|krenem|opišem|pjevam|platim|plešem|plivam|poznam|pričam|skijam|slušam|spavam|stižem|tražim|trebam|čitam|crtam|igram|jedem|kuham|kupim|nemam|pijem|pitam|radim|ronim|ručam|šetam|trčim|vidim|volim|vozim|želim|živim|hoću|idem|imam|učim|znam)|volim ((?:platiti karticom|sljedeći tjedan|ići u kupovinu|fotografirati|imati vremena|kupaći kostim|voziti bicikl|doručkovati|izgubiti se|odmarati se|odmoriti se|planinariti|preporučiti|rezervirati|se izgubiti|se odmarati|se odmoriti|se tuširati|tuširati se|odgovoriti|presjedati|sastati se|se sastati|se sunčati|sunčati se|buditi se|kupati se|nadati se|planirati|posjetiti|poslušati|razumjeti|se buditi|se kupati|se nadati|studirati|govoriti|izlaziti|napisati|naručiti|odabrati|pokazati|ponoviti|putovati|se zvati|skrenuti|ustajati|večerati|vježbati|završiti|zvati se|gledati|kasniti|koštati|krenuti|naći se|opisati|oprosti|pjevati|platiti|plesati|plivati|poznati|pričati|prijeći|pronaći|se naći|skijati|slušati|spavati|tražiti|trebati|vidjeti|voljeti|željeti|živjeti|čitati|crtati|htjeti|igrati|kuhati|kupiti|nemati|pitati|raditi|roniti|ručati|šetati|trčati|voziti|imati|jesti|stići|učiti|znati|piti|ići)|(?:zajedno ili odvojeno|autobusni kolodvor|jednokrevetnu sobu|kremu za sunčanje|dvokrevetnu sobu|osobnu iskaznicu|slobodno vrijeme|hvalu na pomoći|turistički ured|u jednom smjeru|godišnji odmor|mogu li dobiti|nu kraju ulice|povratnu kartu|zu tjedan danu|jednom tjedno|nemam vremenu|nemu problemu|prošli tjedan|stol za dvoje|u koliko sati|hvalu lijepu|kako se kaže|ne razumijem|nemu na čemu|prijateljice|prijateljicu|prijateljiji|svaki tjedan|bijelu kavu|dobro jutro|dobru večer|drago mi je|glavno jelo|informacije|informaciju|ponavljanja|ponavljanje|ponedjeljak|ponedjeljke|putu tjedno|razglednice|razglednicu|rezervacije|rezervaciju|roditeljiji|zvuči super|aktivnoste|cijeli dan|inozemstva|inozemstvo|još jednom|konobarice|konobaricu|plan gradu|prijatelja|prijatelje|sretan put|studentice|studenticu|suncobrane|televizije|televiziju|tjestenine|tjesteninu|učiteljice|učiteljicu|aerodrome|aktivnost|dobar dan|dobar tek|fakultete|gramatike|gramatiku|hoćemo li|inženjera|inženjere|jelovnike|kašnjenja|kašnjenje|kazališta|kazalište|kilograme|kišobrane|knjižnice|knjižnicu|kolodvore|kupaonice|kupaonicu|liječnice|liječnicu|liječnika|liječnike|molim vas|napojnice|napojnicu|novčanike|prodavača|prodavače|putovanja|putovanje|putovnice|putovnicu|radni dan|raskrižja|raskrižje|razgovore|recepcije|recepciju|restorane|roditelje|sladolede|što radiš|što znači|suncobran|svaki dan|to je sve|vidimo se|vozni red|aerodrom|autobuse|blagajne|blagajnu|četvrtak|četvrtke|čokolade|čokoladu|fakultet|gotovine|gotovinu|hoćeš li|hrvatske|hrvatsku|imate li|jelovnik|kako ste|kilogram|kišobran|ključeve|kod kuće|kolegice|kolegicu|kolodvor|koncerte|konobara|konobare|krumpire|laku noć|ležaljke|ležaljku|limunade|limunadu|ljekarne|ljekarnu|nedjelje|nedjelju|nogomete|novčanik|obitelji|odgovore|piletine|piletinu|pomfrite|predjela|predjelo|prognoze|prognozu|proljeća|proljeće|prtljage|prtljagu|računala|računalo|razgovor|rečenice|rečenicu|restoran|roditelj|sastanak|sastanke|semafore|sladoled|slušanja|slušanje|sportove|studenta|studente|suvenire|telefone|teretane|teretanu|trajekte|tramvaje|trgovine|trgovinu|učitelja|učitelje|utakmice|utakmicu|veličine|veličinu|zadatake|autobus|balkone|bicikle|bolnice|bolnicu|bratove|brodove|brojeve|centove|čitanja|čitanje|deserte|djedove|dolaske|dolazak|doručak|doručke|filmove|gradove|javi se|jogurte|kartice|karticu|koncert|košarke|košarku|kruhove|krumpir|laptope|lekcije|lekciju|liftove|lozinke|lozinku|maslace|mjesece|mlijeka|mlijeko|mostove|naočala|naočale|ne mogu|ne znam|nogomet|nu piće|nu uglu|obitelj|odbojke|odbojku|odgovor|parkove|pijesak|pijeske|pisanja|pisanje|pitanja|pitanje|planine|planinu|planove|polaske|polazak|poljske|poljsku|pomfrit|prijave|prijavu|priloge|putnika|putnike|ručnike|ruksake|semafor|snijege|srijede|srijedu|stanice|stanicu|stanove|stolove|stupanj|stupnje|susjeda|susjede|susjedu|suvenir|taksiji|tanjure|telefon|testove|trajekt|tramvaj|tržnice|tržnicu|turista|turiste|učenika|učenike|ukrcaje|vikende|vlakove|vrećice|vrećicu|vrijema|vrijeme|zadatak|adrese|adresu|avione|balkon|bazene|bicikl|čajeve|centar|centre|cijene|cijenu|danove|desert|dijete|glazbe|glazbu|godine|godinu|govore|hobiji|hotele|igrice|igricu|izlaze|izlete|jabuke|jabuku|jeseni|jezike|jogurt|kafiće|katove|knjige|knjigu|kofere|kolače|kolege|kolegu|komade|laptop|ledove|letove|maslac|minute|minutu|mjesec|mjesta|mjesto|muzeje|muževe|noževe|odjave|odjavu|odmore|pekare|pekaru|perone|pomoći|ponoći|poruke|poruku|povrća|povrće|prilog|putove|račune|riječi|ručnik|ruksak|salate|salatu|šalice|šalicu|satove|šećere|serije|seriju|sestre|sestru|sinove|sirove|snijeg|sokove|subote|subotu|tanjur|tečaje|tenise|tjedan|tjedne|tvrtke|tvrtku|u šest|ukrcaj|utorak|utorke|valove|večere|večeru|vikend|vilice|vilicu|vjetar|vjetre|zabave|zabavu|zadaće|zadaću|zemlje|zemlju|zgrade|zgradu|avion|banke|banku|bazen|brata|crkve|crkvu|djece|djecu|djeda|govor|hotel|hrane|hranu|izlaz|izlet|jesen|jezik|kafić|karte|kartu|klime|klimu|ključ|kofer|kolač|komad|ljeta|ljeto|mačke|mačku|magle|maglu|majke|majku|muzej|novac|novce|obale|obalu|odmor|oluje|oluju|osobe|osobu|otoke|peron|petak|petke|plaže|plažu|podna|podne|pomoć|ponoć|posaa|posao|pošte|poštu|račun|riječ|ručak|ručke|šećer|škole|školu|sport|sunca|sunce|šunke|šunku|taksi|tečaj|tenis|torbe|torbu|ulaze|ulice|ulicu|urede|večer|žlice|žlicu|auta|auto|bake|baku|boce|bocu|brod|broj|čaše|čašu|cent|eura|euro|film|grad|hobi|i ju|jaja|jaje|jela|jelo|joge|jogu|juhe|juhu|kave|kavu|kćer|kćii|kina|kino|kiše|kišu|kruh|kuće|kuću|lift|mame|mamu|mesa|meso|mora|more|most|muža|noći|otok|park|pića|piće|piva|pivo|plan|pole|polu|ribe|ribu|riže|rižu|sina|sobe|sobu|stan|stol|tate|tatu|test|trge|u ti|ulaz|ured|vina|vino|vlak|voća|voće|vode|vodu|vrte|žene|ženu|zime|zimu|čaj|dan|ima|ime|kat|led|let|noć|nož|oca|oce|psa|pse|put|sat|sir|sok|trg|val|vrt)))( \\p{L}+){0,3}( (navečer|obično|često|ponekad|uvijek|rijetko))*|((navečer|obično|često|ponekad|uvijek|rijetko) )*(ja )?se ((navečer|obično|često|ponekad|uvijek|rijetko) )+(?:idem u kupovinu|platim karticom|fotografiram|imam vremena|preporučujem|vozim bicikl|doručkujem|planinarim|preporučim|rezerviram|odgovorim|presjedam|razumijem|odaberem|planiram|posjetim|poslušam|poznajem|prijeđem|pronađem|sastanem|studiram|govorim|izgubim|izlazim|napišem|naručim|odmaram|odmorim|pokažem|ponovim|putujem|skrenem|stignem|tuširam|ustajem|večeram|vježbam|završim|gledam|kasnim|koštam|krenem|opišem|pjevam|platim|plešem|plivam|poznam|pričam|skijam|slušam|spavam|stižem|sunčam|tražim|trebam|budim|čitam|crtam|igram|jedem|kuham|kupam|kupim|nadam|nađem|nemam|pijem|pitam|radim|ronim|ručam|šetam|trčim|vidim|volim|vozim|želim|živim|zovem|hoću|idem|imam|učim|znam)( \\p{L}+){0,3})(?: (?:hvala(?: lijepa| vam)?|a ti|a vi|molim(?: vas)?|marko))*$",
            "suggestion": "Navečer se obično odmaram.",
            "suggestionAudioSrc": "/audio/hr/a1/module-03/navecer-se-obicno-odmaram.mp3"
>>>>>>> e88cd64 (Generaownie voice)
          }
        ]
      },
      {
        "id": "can-do",
        "stage": "dialog",
        "type": "free",
        "instruction": "Opisz swój zwykły tydzień w 2–4 zdaniach.",
        "points": [
          "co robisz codziennie",
          "kiedy pracujesz lub się uczysz",
          "co robisz w wolny dzień"
        ],
        "keywords": [
          {
            "any": [
              "svaki",
              "obično",
              "ponekad",
              "često"
            ],
            "label": "jak często"
          },
          {
            "any": [
              "radim",
              "učim"
            ],
            "label": "praca lub nauka"
          },
          {
            "any": [
              "subotu",
              "nedjelju",
              "navečer",
              "ujutro"
            ],
            "label": "kiedy"
          }
        ],
        "minSentences": 2,
        "sample": "Svaki dan učim hrvatski. Obično radim od deset. U subotu sam slobodan.",
        "sampleAudioSrc": "/audio/hr/a1/module-03/svaki-dan-ucim-hrvatski-obicno-radim-od-deset-u-subotu-sam-s.mp3"
      },
      {
        "id": "summary",
        "stage": "summary",
        "type": "summary",
        "title": "Lekcja ukończona",
        "recap": [
          {
            "target": "Svaki dan učim hrvatski.",
            "source": "Codziennie uczę się chorwackiego.",
            "audioSrc": "/audio/hr/a1/module-03/svaki-dan-ucim-hrvatski.mp3"
          },
          {
            "target": "Obično radim od deset.",
            "source": "Zwykle pracuję od dziesiątej.",
            "audioSrc": "/audio/hr/a1/module-03/obicno-radim-od-deset.mp3"
          },
          {
            "target": "Ponekad idem u teretanu.",
            "source": "Czasami chodzę na siłownię.",
            "audioSrc": "/audio/hr/a1/module-03/ponekad-idem-u-teretanu.mp3"
          },
          {
            "target": "U subotu sam slobodan.",
            "source": "W sobotę jestem wolny.",
            "audioSrc": "/audio/hr/a1/module-03/u-subotu-sam-slobodan.mp3"
          }
        ],
        "canDo": [
          "opowiedzieć prostymi zdaniami o codziennych czynnościach",
          "powiedzieć, gdzie pracujesz i czego się uczysz",
          "zapytać o godzinę i podać godzinę",
          "powiedzieć, co robisz dziś, jutro i w wybrany dzień"
        ]
      }
    ]
  },
  material: {
    "sourceLessonId": "a1-15",
    "grammarFocus": "utrwalenie czasu teraźniejszego i określeń czasu",
    "communicativeGoal": "Połączyć rutynę, pracę, naukę i czas.",
    "sources": [
      "https://www.coe.int/en/web/common-European-framework-reference-languages/table-1-cefr-3.3-common-reference-levels-global-scale",
      "https://www.coe.int/en/web/common-european-framework-reference-languages/cefr-descriptors-search",
      "https://croaticum.ffzg.unizg.hr/?page_id=1106",
      "https://a1.ffzg.unizg.hr/",
      "https://www.easy-croatian.com/2014/11/a3.html"
    ],
    "records": [
      {
        "recordId": "A1-0253",
        "type": "lesson",
        "sequence": 0,
        "hr": "",
        "pl": "",
        "lemma": "",
        "partOfSpeech": "",
        "exerciseType": "",
        "acceptedAnswers": [],
        "notes": "Projekt autorski oparty na deskryptorach CEFR A1 oraz zakresie początkujących kursów Croaticum. Treści nie są kopią podręcznika.",
        "tags": [
          "curriculum",
          "cefr-a1",
          "croatian"
        ]
      },
      {
        "recordId": "A1-0254",
        "type": "vocabulary",
        "sequence": 1,
        "hr": "tjedan",
        "pl": "tydzień",
        "lemma": "tjedan",
        "partOfSpeech": "noun",
        "exerciseType": "flashcard|recognition|production",
        "acceptedAnswers": [
          "tjedan"
        ],
        "notes": "Docelowy element aktywnego słownictwa; wrzuć do FSRS po ukończeniu lekcji.",
        "tags": [
          "vocab",
          "active"
        ]
      },
      {
        "recordId": "A1-0255",
        "type": "vocabulary",
        "sequence": 2,
        "hr": "svaki dan",
        "pl": "codziennie",
        "lemma": "svaki dan",
        "partOfSpeech": "phrase",
        "exerciseType": "flashcard|recognition|production",
        "acceptedAnswers": [
          "svaki dan"
        ],
        "notes": "Docelowy element aktywnego słownictwa; wrzuć do FSRS po ukończeniu lekcji.",
        "tags": [
          "vocab",
          "active"
        ]
      },
      {
        "recordId": "A1-0256",
        "type": "vocabulary",
        "sequence": 3,
        "hr": "obično",
        "pl": "zwykle",
        "lemma": "obično",
        "partOfSpeech": "adverb",
        "exerciseType": "flashcard|recognition|production",
        "acceptedAnswers": [
          "obično"
        ],
        "notes": "Docelowy element aktywnego słownictwa; wrzuć do FSRS po ukończeniu lekcji.",
        "tags": [
          "vocab",
          "active"
        ]
      },
      {
        "recordId": "A1-0257",
        "type": "vocabulary",
        "sequence": 4,
        "hr": "ponekad",
        "pl": "czasami",
        "lemma": "ponekad",
        "partOfSpeech": "adverb",
        "exerciseType": "flashcard|recognition|production",
        "acceptedAnswers": [
          "ponekad"
        ],
        "notes": "Docelowy element aktywnego słownictwa; wrzuć do FSRS po ukończeniu lekcji.",
        "tags": [
          "vocab",
          "active"
        ]
      },
      {
        "recordId": "A1-0258",
        "type": "vocabulary",
        "sequence": 5,
        "hr": "rano",
        "pl": "wcześnie",
        "lemma": "rano",
        "partOfSpeech": "adverb",
        "exerciseType": "flashcard|recognition|production",
        "acceptedAnswers": [
          "rano"
        ],
        "notes": "Docelowy element aktywnego słownictwa; wrzuć do FSRS po ukończeniu lekcji.",
        "tags": [
          "vocab",
          "active"
        ]
      },
      {
        "recordId": "A1-0259",
        "type": "vocabulary",
        "sequence": 6,
        "hr": "kasno",
        "pl": "późno",
        "lemma": "kasno",
        "partOfSpeech": "adverb",
        "exerciseType": "flashcard|recognition|production",
        "acceptedAnswers": [
          "kasno"
        ],
        "notes": "Docelowy element aktywnego słownictwa; wrzuć do FSRS po ukończeniu lekcji.",
        "tags": [
          "vocab",
          "active"
        ]
      },
      {
        "recordId": "A1-0260",
        "type": "vocabulary",
        "sequence": 7,
        "hr": "plan",
        "pl": "plan",
        "lemma": "plan",
        "partOfSpeech": "noun",
        "exerciseType": "flashcard|recognition|production",
        "acceptedAnswers": [
          "plan"
        ],
        "notes": "Docelowy element aktywnego słownictwa; wrzuć do FSRS po ukończeniu lekcji.",
        "tags": [
          "vocab",
          "active"
        ]
      },
      {
        "recordId": "A1-0261",
        "type": "vocabulary",
        "sequence": 8,
        "hr": "slobodan",
        "pl": "wolny",
        "lemma": "slobodan",
        "partOfSpeech": "adjective",
        "exerciseType": "flashcard|recognition|production",
        "acceptedAnswers": [
          "slobodan"
        ],
        "notes": "Docelowy element aktywnego słownictwa; wrzuć do FSRS po ukończeniu lekcji.",
        "tags": [
          "vocab",
          "active"
        ]
      },
      {
        "recordId": "A1-0845",
        "type": "vocabulary",
        "sequence": 9,
        "hr": "rijetko",
        "pl": "rzadko",
        "lemma": "rijetko",
        "partOfSpeech": "adverb",
        "exerciseType": "flashcard|recognition",
        "acceptedAnswers": [
          "rijetko"
        ],
        "notes": "Słownictwo uzupełniające: lista „Więcej przydatnych słów” w lekcji, nagranie i fiszki; bez osobnej karty słowa.",
        "tags": [
          "vocab",
          "supplement"
        ]
      },
      {
        "recordId": "A1-0846",
        "type": "vocabulary",
        "sequence": 10,
        "hr": "jednom tjedno",
        "pl": "raz w tygodniu",
        "lemma": "tjedno",
        "partOfSpeech": "phrase",
        "exerciseType": "flashcard|recognition",
        "acceptedAnswers": [
          "jednom tjedno"
        ],
        "notes": "Słownictwo uzupełniające: lista „Więcej przydatnych słów” w lekcji, nagranie i fiszki; bez osobnej karty słowa.",
        "tags": [
          "vocab",
          "supplement"
        ]
      },
      {
        "recordId": "A1-0847",
        "type": "vocabulary",
        "sequence": 11,
        "hr": "svaki tjedan",
        "pl": "co tydzień",
        "lemma": "tjedan",
        "partOfSpeech": "phrase",
        "exerciseType": "flashcard|recognition",
        "acceptedAnswers": [
          "svaki tjedan"
        ],
        "notes": "Słownictwo uzupełniające: lista „Więcej przydatnych słów” w lekcji, nagranie i fiszki; bez osobnej karty słowa.",
        "tags": [
          "vocab",
          "supplement"
        ]
      },
      {
        "recordId": "A1-0848",
        "type": "vocabulary",
        "sequence": 12,
        "hr": "mjesec",
        "pl": "miesiąc",
        "lemma": "mjesec",
        "partOfSpeech": "noun",
        "exerciseType": "flashcard|recognition",
        "acceptedAnswers": [
          "mjesec"
        ],
        "notes": "Słownictwo uzupełniające: lista „Więcej przydatnych słów” w lekcji, nagranie i fiszki; bez osobnej karty słowa.",
        "tags": [
          "vocab",
          "supplement"
        ]
      },
      {
        "recordId": "A1-0849",
        "type": "vocabulary",
        "sequence": 13,
        "hr": "radni dan",
        "pl": "dzień roboczy",
        "lemma": "dan",
        "partOfSpeech": "phrase",
        "exerciseType": "flashcard|recognition",
        "acceptedAnswers": [
          "radni dan"
        ],
        "notes": "Słownictwo uzupełniające: lista „Więcej przydatnych słów” w lekcji, nagranie i fiszki; bez osobnej karty słowa.",
        "tags": [
          "vocab",
          "supplement"
        ]
      },
      {
        "recordId": "A1-0262",
        "type": "sentence",
        "sequence": 1,
        "hr": "Svaki dan učim hrvatski.",
        "pl": "Codziennie uczę się chorwackiego.",
        "lemma": "",
        "partOfSpeech": "",
        "exerciseType": "translation|word_order|listening|gap",
        "acceptedAnswers": [
          "Svaki dan učim hrvatski."
        ],
        "notes": "Zdanie autorskie do ćwiczeń; przy inputach tekstowych warto akceptować brak części znaków diakrytycznych jako soft error, ale pokazać poprawną pisownię.",
        "tags": [
          "sentence",
          "production",
          "comprehension"
        ]
      },
      {
        "recordId": "A1-0263",
        "type": "sentence",
        "sequence": 2,
        "hr": "Obično radim od deset.",
        "pl": "Zwykle pracuję od dziesiątej.",
        "lemma": "",
        "partOfSpeech": "",
        "exerciseType": "translation|word_order|listening|gap",
        "acceptedAnswers": [
          "Obično radim od deset."
        ],
        "notes": "Zdanie autorskie do ćwiczeń; przy inputach tekstowych warto akceptować brak części znaków diakrytycznych jako soft error, ale pokazać poprawną pisownię.",
        "tags": [
          "sentence",
          "production",
          "comprehension"
        ]
      },
      {
        "recordId": "A1-0264",
        "type": "sentence",
        "sequence": 3,
        "hr": "Ponekad idem u teretanu.",
        "pl": "Czasami chodzę na siłownię.",
        "lemma": "",
        "partOfSpeech": "",
        "exerciseType": "translation|word_order|listening|gap",
        "acceptedAnswers": [
          "Ponekad idem u teretanu."
        ],
        "notes": "Zdanie autorskie do ćwiczeń; przy inputach tekstowych warto akceptować brak części znaków diakrytycznych jako soft error, ale pokazać poprawną pisownię.",
        "tags": [
          "sentence",
          "production",
          "comprehension"
        ]
      },
      {
        "recordId": "A1-0265",
        "type": "sentence",
        "sequence": 4,
        "hr": "U subotu sam slobodan.",
        "pl": "W sobotę jestem wolny.",
        "lemma": "",
        "partOfSpeech": "",
        "exerciseType": "translation|word_order|listening|gap",
        "acceptedAnswers": [
          "U subotu sam slobodan."
        ],
        "notes": "Zdanie autorskie do ćwiczeń; przy inputach tekstowych warto akceptować brak części znaków diakrytycznych jako soft error, ale pokazać poprawną pisownię.",
        "tags": [
          "sentence",
          "production",
          "comprehension"
        ]
      },
      {
        "recordId": "A1-0266",
        "type": "sentence",
        "sequence": 5,
        "hr": "Navečer se obično odmaram.",
        "pl": "Wieczorem zwykle odpoczywam.",
        "lemma": "",
        "partOfSpeech": "",
        "exerciseType": "translation|word_order|listening|gap",
        "acceptedAnswers": [
          "Navečer se obično odmaram."
        ],
        "notes": "Zdanie autorskie do ćwiczeń; przy inputach tekstowych warto akceptować brak części znaków diakrytycznych jako soft error, ale pokazać poprawną pisownię.",
        "tags": [
          "sentence",
          "production",
          "comprehension"
        ]
      },
      {
        "recordId": "A1-0267",
        "type": "sentence",
        "sequence": 6,
        "hr": "Ovo je moj plan za tjedan.",
        "pl": "To jest mój plan na tydzień.",
        "lemma": "",
        "partOfSpeech": "",
        "exerciseType": "translation|word_order|listening|gap",
        "acceptedAnswers": [
          "Ovo je moj plan za tjedan."
        ],
        "notes": "Zdanie autorskie do ćwiczeń; przy inputach tekstowych warto akceptować brak części znaków diakrytycznych jako soft error, ale pokazać poprawną pisownię.",
        "tags": [
          "sentence",
          "production",
          "comprehension"
        ]
      },
      {
        "recordId": "A1-0850",
        "type": "sentence",
        "sequence": 7,
        "hr": "Rijetko radim vikendom.",
        "pl": "Rzadko pracuję w weekendy.",
        "lemma": "",
        "partOfSpeech": "",
        "exerciseType": "listening|comprehension",
        "acceptedAnswers": [
          "Rijetko radim vikendom."
        ],
        "notes": "Zdanie przykładowe z rozszerzenia modułów 02–08; pokazywane z nagraniem w kroku „Nowe słowa w zdaniach”.",
        "tags": [
          "sentence",
          "example",
          "supplement"
        ]
      },
      {
        "recordId": "A1-0851",
        "type": "sentence",
        "sequence": 8,
        "hr": "Jednom tjedno idem na bazen.",
        "pl": "Raz w tygodniu chodzę na basen.",
        "lemma": "",
        "partOfSpeech": "",
        "exerciseType": "listening|comprehension",
        "acceptedAnswers": [
          "Jednom tjedno idem na bazen."
        ],
        "notes": "Zdanie przykładowe z rozszerzenia modułów 02–08; pokazywane z nagraniem w kroku „Nowe słowa w zdaniach”.",
        "tags": [
          "sentence",
          "example",
          "supplement"
        ]
      },
      {
        "recordId": "A1-0268",
        "type": "exercise_blueprint",
        "sequence": 1,
        "hr": "",
        "pl": "",
        "lemma": "",
        "partOfSpeech": "",
        "exerciseType": "mixed_review",
        "acceptedAnswers": [],
        "notes": "Losuj materiał z 4 poprzednich lekcji: wybór, luka, tłumaczenie, kolejność słów.",
        "tags": [
          "exercise",
          "lesson-player"
        ]
      },
      {
        "recordId": "A1-0269",
        "type": "exercise_blueprint",
        "sequence": 2,
        "hr": "",
        "pl": "",
        "lemma": "",
        "partOfSpeech": "",
        "exerciseType": "dialog_completion",
        "acceptedAnswers": [],
        "notes": "Uzupełnij 3–5 tur dialogu bez podpowiadania pełnych odpowiedzi.",
        "tags": [
          "exercise",
          "lesson-player"
        ]
      },
      {
        "recordId": "A1-0270",
        "type": "exercise_blueprint",
        "sequence": 3,
        "hr": "",
        "pl": "",
        "lemma": "",
        "partOfSpeech": "",
        "exerciseType": "can_do_task",
        "acceptedAnswers": [],
        "notes": "Wykonaj zadanie komunikacyjne modułu w 2–4 zdaniach.",
        "tags": [
          "exercise",
          "lesson-player"
        ]
      }
    ]
  },
};
